...
class Retriever:
    def __init__(self, embedder: Optional[SimpleTfidfEmbedder] = None, vector_store: Optional[SQLiteVectorStore] = None, embedder_path: str | None = None, store_path: str | None = None) -> None:
        ...
        self.store_path = os.path.abspath(store_path or "./data/vector_store.sqlite")
        self.embedder_path = os.path.abspath(embedder_path or "./data/embedder.pkl")
        self.vector_store = vector_store or SQLiteVectorStore(self.store_path)
        print(f"[Retriever] Initialized with store_path: {self.store_path}")
        print(f"[Retriever] Embedder path: {self.embedder_path}")

    def build_index(self, chunks: List[Dict[str, object]]) -> None:
        print(f"[Retriever] Building index for {len(chunks)} chunks")
        ...
        print(f"[Retriever] Index built and saved to {self.embedder_path}")
