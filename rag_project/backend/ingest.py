...
class DocumentIngestor:
    def __init__(self, data_dir: str = "./data", vector_db_path: str | None = None, embedder_path: str | None = None) -> None:
        ...
        self.vector_db_path = vector_db_path or os.path.join(self.data_dir, "vector_store.sqlite")
        self.embedder_path = embedder_path or os.path.join(self.data_dir, "embedder.pkl")
        ...
        print(f"[Ingestor] Initialized with data_dir: {self.data_dir}")
        print(f"[Ingestor] Vector DB path: {self.vector_db_path}")
        print(f"[Ingestor] Embedder path: {self.embedder_path}")

    def ingest_directory(self, directory: str) -> Dict[str, Any]:
        print(f"[Ingestor] Starting ingestion from directory: {directory}")
        documents = []
        for path in Path(directory).rglob('*'):
            ...
            print(f"[Ingestor] Processing file: {path.name}")
            chunks = self.chunker.ingest_file(str(path), document_id=document_id, metadata=metadata)
            documents.extend(chunks)
            print(f"[Ingestor] Generated {len(chunks)} chunks from {path.name}")
        print(f"[Ingestor] Building index for {len(documents)} total chunks")
        self.retriever.build_index(documents)
        result = {"documents_ingested": len({chunk["document_id"] for chunk in documents}), "chunks": len(documents)}
        print(f"[Ingestor] Ingestion complete: {result}")
        return result
